...
- `FaceRecognizer`：负责摄像头采集、活体检测、特征提取与特征比对
...
| 多种本地通信方式 | 使用共享内存与 UDP 传递图像、状态和识别结果 |
...
- `FaceRecognizer.exe` 负责识别相关任务，例如抓拍、预览流、特征提取和特征比较。
...
仓库已经包含 Inno Setup 脚本 [`setup.iss`](setup.iss)，其职责主要包括：
