...
impl<T> Drop for MemSafeRead<'_, T> {
    fn drop(&mut self) {
        // Fail-secure (project patch): never panic from Drop. If revoking
        // privileges fails the region stays at its current protection level;
        // a panic here during unwinding would abort the process and could
        // leave the region readable. The caller cannot observe this state,
        // so no secret-bearing diagnostic is recorded.
        let _ = self.mem_safe.cell.low_priv();
    }
}
...
impl<T> Drop for MemSafeWrite<'_, T> {
    fn drop(&mut self) {
        // Fail-secure (project patch): see MemSafeRead::drop.
        let _ = self.mem_safe.cell.low_priv();
    }
}
