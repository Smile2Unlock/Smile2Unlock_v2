...
path executable_dir() {
    ...
    return length == 0 ? std::filesystem::current_path() : path(std::string_view(buffer.data(), length)).parent_path();
#else
    std::array<char, 4096> buffer{};
    const auto length = readlink("/proc/self/exe", buffer.data(), buffer.size());
    return length <= 0 ? std::filesystem::current_path() : path(std::string_view(buffer.data(), static_cast<std::size_t>(length))).parent_path();
#endif
}
