...
bool Database::Initialize(const std::string& db_path) {
    ...
    // Legacy AES-CBC passwords used a key stored beside the database. They are
    // deliberately retired instead of silently decrypted into the new store.
    if (sqlite3_exec(
            db_, "UPDATE users SET encrypted_password = NULL ",
            nullptr, nullptr, nullptr) != SQLITE_OK) {
        return false;
    }
    auto error = std::error_code{};
    (void)std::filesystem::remove(smile2unlock::paths::GetDatabaseKeyPath(), error);
    return true;
}
...
std::vector<User> Database::GetAllUsers() const {
    ...
    const char* sql = "SELECT id, username, remark, created_at FROM users;";
    sqlite3_stmt* stmt;
    ...
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) == SQLITE_OK) {
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            const char* rmk = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 2));
            if (rmk) u.remark = rmk;
...
std::optional<User> Database::GetUserById(int id) const {
    const char* sql = "SELECT id, username, remark, created_at FROM users WHERE id = ?;";
    sqlite3_stmt* stmt;
...
std::optional<User> Database::GetUserByUsername(const std::string& username) const {
    const char* sql = "SELECT id, username, remark, created_at FROM users WHERE username = ?;";
    sqlite3_stmt* stmt;
    std::optional<User> user = std::nullopt;
...
bool Database::AddUser(const User& user) {
    ...
    const char* sql = "INSERT INTO users (username, remark, created_at) VALUES (?, ?, ?);";
    sqlite3_stmt* stmt;
    bool success = false;

    if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_text(stmt, 1, user.username.c_str(), -1, SQLITE_TRANSIENT);
        sqlite3_bind_text(stmt, 2, user.remark.c_str(), -1, SQLITE_TRANSIENT);
...
bool Database::UpdateUser(const User& user) {
    const char* sql = "UPDATE users SET username=?, remark=? WHERE id=?;";
    sqlite3_stmt* stmt;
    bool success = false;
